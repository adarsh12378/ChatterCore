...
if(members.length<2) return next(new ErrorHandler("Group chat must have at least 3 member",400)
);
...
const getMyChats=TryCatch(async(req,res,next)=>{
    ...
    const transFormChats=chats.map(({_id,name,members,groupChat})=>{
        ...
        return {
            ...
            avatar:groupChat?members.slice(0,3).map(({avatar})=>avatar.url):otherMember.url
        };
    });
    return res.status(201).json({
        success:true,
    ...
    const getMyGroups = TryCatch(async (req, res, next) => {
      ...
      const addMembers = TryCatch(async (req, res, next) => {
        ...
        if(!members || members.length<1) return next(
           new  ErrorHandler("Please Provide Members",400)
        );
